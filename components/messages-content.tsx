...
export function MessagesContent() {
  ...
  const loadConversations = async (silent = false) => {
    try {
      ...
      const result:any = await getUserConversations();
  ...
  const loadMessages = async (conversationId: string, silent = false) => {
    try {
      ...
      const newMessages:any = result.messages || [];
      if (silent) {
        setActiveMessages((prevMessages:any) => {
          // Only update if the message count or content has changed
          if (prevMessages.length !== newMessages.length || 
              JSON.stringify(prevMessages.map((m:any )=> m.id)) !== JSON.stringify(newMessages.map((m:any )=> m.id))) {
            return newMessages;
          }
          return prevMessages;
        });
